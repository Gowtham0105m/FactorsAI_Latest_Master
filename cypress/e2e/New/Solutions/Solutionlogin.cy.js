describe('SolutionLogin', () => {
    ...
    it('SolutionLoginPage', () => {
        ...
        cy.wait(5000)
        cy.wait(5000)
        cy.xpath('//h4[contains(@class,"text-base font-normal")]').should('be.visible').and('contain.text', 'All Accounts')
        cy.get('#fa-at-dropdown--settings').click()
        cy.xpath('//h4[contains(@class,"text-base font-normal")]').should('be.visible').and('contain.text', 'General')
        cy.xpath('//span[text()="Touchpoints Definitions"]').click()
        cy.xpath('//span[text()="Channel Groups"]').click()
        cy.xpath('//th[text()="Conditions"]').should('be.visible').and('contain.text', 'Conditions')
        cy.xpath('//span[text()="Add New"]').click()
        cy.xpath('//div[text()="Add channel group"]').should('be.visible').and('contain.text', 'Add channel group')
